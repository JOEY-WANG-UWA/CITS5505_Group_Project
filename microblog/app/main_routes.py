...
def add_to_collection(upload_id):
    try:
        ...
        print(
            f"Received request to add to collection: upload_id={upload_id}, user_id={current_user.id}")
...
def search():
    ...
    if request.args.get('query'):
        ...
        filtered_details = {upload_id: details for upload_id, details in grouped_details.items(
        ) if upload_id in [upload.id for upload in uploads]}
    ...
    return render_template('search.html', title='search', form=form, grouped_details=filtered_details, comments_with_user=comments_with_user)
